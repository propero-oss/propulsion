...
Filters.checkSupport = function<T = any, F extends keyof T = any>(filter: Filter<T, F>, supported: FilterOperator[]): boolean {
  if (!supported.includes(filter.op)) return false;
  if (filter.op === "and" || filter.op === "or") return filter.filters.every(filter => Filters.checkSupport(filter, supported));
  if (filter.op === "not") return Filters.checkSupport(filter.filter, supported);
  return true;
};
