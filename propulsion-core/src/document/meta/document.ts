...
export interface DocumentMeta<T = any> {
...
export function Document(data?: Omit<DocumentMeta, "fields">, scope = "main") {
  return function(target: NoArgsConstructor) {
    ...
    if (!Reflect.hasMetadata(DOCUMENT, target)) Reflect.defineMetadata(DOCUMENT, {}, target);
    Reflect.getMetadata(DOCUMENT, target)[scope] = { ...data, fields };
    return target;
  };
}

Document.getMeta = function<T extends NoArgsConstructor>(target: T, scope = "main"): DocumentMeta<T> {
