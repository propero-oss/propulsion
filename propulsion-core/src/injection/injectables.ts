...
export class InjectableRegistry {
  ...
  public getInjectable(dep: any, instance?: any) {
    const {passInstance} = this.getInjectableMeta(dep);
    if (passInstance && !instance)
      throw new Error(`Cannot instantiate injectable, injectable needs an instance argument.`);
    return passInstance
      ? (this.createFactory(dep) as SingleArgFunction)(instance)
      : (this.createFactory(dep) as NoArgsFunction)();
  }

  public createFactory(dep: any): NoArgsFunction | SingleArgFunction {


    const {factory, cls, instance, passInstance, multiInstance, id} = this.getInjectableMeta(dep);

    if (instance)
      return () => instance;

    if (cls && multiInstance)
      return () => new cls();

    if (cls)
      return () => this.instanciate(cls);
  ...
  getInjectableMeta(dep: any) {
    ...
    if (!this.hasInjectable(id))
      throw new Error(`No injectable registered for ${String(dep)} (id: ${String(id)})`);
