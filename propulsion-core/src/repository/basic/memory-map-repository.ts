...
import { FetchOptions, SingleFetchOptions } from "@/repository/fetch-options";
import { NoSuchElement } from "@/repository/error/no-such-element";
import { UnsupportedOperation } from "@/repository/error/unsupported-operation";
...
export class MemoryMapRepository<T, F extends keyof T, ID extends T[F] = T[F]> extends RepositoryBase<T, F, ID> {
  ...
  constructor(cls: NoArgsConstructor<T>, id: F) {
    super(cls, id);
  }

  public async findOne(id: ID, options?: SingleFetchOptions<T>): Promise<T> {
    let result = this.state.get(id);
    if (!result) throw new NoSuchElement(`No element found for id ${id}`);

    if (options && options.fields) [result] = this.transformFields([result], options.fields);
  ...
  public async findAll(options?: FetchOptions<T>): Promise<T[]> {
    ...
    if (filter) entries = this.filterEntries(entries, filter);
    if (fields) entries = this.transformFields(entries, fields);
    if (sort) entries = this.sortEntries(entries, sort);
  ...
  public async updateOne(entity: Partial<T>, partialUpdate?: boolean): Promise<T> {
    ...
    if (!this.state.has(id)) throw new NoSuchElement(`No element found for id ${id}`);
  ...
  public async deleteOne(id: ID): Promise<void> {
    if (this.state.has(id)) this.state.delete(id);
  }
  ...
  protected createFilterFunction(filter: Filter<T, keyof T>): (data: T) => boolean {
    switch (filter.op) {
      case "and":
        return this.createAndFunction(filter.filters);
      case "or":
        return this.createOrFunction(filter.filters);
      case "not":
        return this.createFilterFunction(Filters.negate(filter.filter));

      case "in":
        return (data: T) => filter.items.includes(data[filter.field]);
      case "not-in":
        return (data: T) => !filter.items.includes(data[filter.field]);

      case "null":
        return (data: T) => data[filter.field] == null;
      case "not-null":
        return (data: T) => data[filter.field] != null;

      case "equals":
        return (data: T) => data[filter.field] === filter.value;
      case "not-equal":
        return (data: T) => data[filter.field] !== filter.value;
      ...
      case "contains":
        return (data: T) => String(data[filter.field]).includes(String(filter.value));
      case "not-contains":
        return (data: T) => !String(data[filter.field]).includes(String(filter.value));

      case "starts-with":
        return (data: T) => String(data[filter.field]).startsWith(String(filter.value));
      case "not-starts-with":
        return (data: T) => !String(data[filter.field]).startsWith(String(filter.value));

      case "ends-with":
        return (data: T) => String(data[filter.field]).endsWith(String(filter.value));
      case "not-ends-with":
        return (data: T) => !String(data[filter.field]).endsWith(String(filter.value));

      case "match":
        return (data: T) => new RegExp(String(filter.value)).test(String(data[filter.field]));
      case "not-match":
        return (data: T) => !new RegExp(String(filter.value)).test(String(data[filter.field]));
