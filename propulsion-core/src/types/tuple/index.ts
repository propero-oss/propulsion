 ...
 */
export type TuplePush<T extends any[], V, L = TuplePushHelperType[T["length"]], P = { [K in keyof L]: K extends keyof T ? T[K] : V }> = P extends any[]
  ? P
  : never;
 ...
 * Only works for first 100 elements
 */
export type TuplePop<T extends any[], L = TuplePopHelperType[T["length"]], P = { [K in keyof L]: K extends keyof T ? T[K] : never }> = P extends any[]
  ? P
  : never;
 ...
 */
export type TupleUnshift<TUPLE extends [...any[]], ELEMENT> = ((e: ELEMENT, ...r: TUPLE) => void) extends (...a: infer R) => any ? R : never;
