...
export function normalizeOptions(options: HandlerOptions): ProcessedHandlerOptions {
  ...
  if (options.headers) processed.headers = optimizeRequestHeaders(normalizeRequestHeaders(options.headers));

  if (options.method) processed.method = createMethodCondition(normalizeRequestMethods(options.method));
